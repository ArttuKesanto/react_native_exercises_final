// Important: there are a lot of code-lines here for future work, that is why this section is so crowded - the main functionality works.
import React,
...
export default function GetAddressFromBook({route}) {
    const {addressString} = route.params


    //const [address, setAddress] = useState(addressString)
    const url = ("https://www.mapquestapi.com/geocoding/v1/address?key=[YOUR KEY HERE.]&inFormat=kvp&outFormat=json&location=" + addressString + "&thumbMaps=false");
    ...
    const fetchAddress = async () => { // Solved the re-render issues. For future work here. Not in use now.
    ...
    useEffect(() => {
        fetchNewAddress();
    }, [addressString]) // Re-renders every time a new props is gained from AddressBook.jsx.
